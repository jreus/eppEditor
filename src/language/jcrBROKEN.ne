# Something happened in this language spec that led to increasingly
# severe issues with the Google Helper Process using up all CPU
# and the tree->code conversion function going into what looked like an insane loop.
# Eventually became unusable and had to start over..
...
Statement ->
      ...
      # | %hash . "\n"                                          {% d => ({ "@comment": d[3] }) %}
      |
      Expression _ %semicolon                       {% d => [{"@sigOut": { "@spawn": d[0] }}] %}
...
Expression ->
  ...
  |
  %variable _ %equals _ Expression
